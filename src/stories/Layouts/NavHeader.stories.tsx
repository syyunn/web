...
export const NavigationHeader: FunctionComponent<NavigationHeaderProps> = ({
    ...
    const nav_cx = classNames(
        `z-4 top-0 bb w-100 bg-${bgColor} b--${bbColor}`,
    );
    const div_cx = classNames('w-100 ph4-l center flex justify-between items-center pv2 overflow-y-auto h-auto');
...
export const NavigationHeaderWithTextLogoWithoutBorderBottomLine = () => NavigationHeader({ height: 70, logo: Heading({ text: "DeepWTO : A Deep Learning Approach for Predicting Invokable Articles in WTO" }), bgColor: "white", bbColor: 'silver', text: Heading({ text: "Level 1 Heading" }) });
